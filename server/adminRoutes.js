...
router.get('/submissions', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const submissionsWithGeo = enrichedSubmissions;
    ...
    const uniqueUserAgents = Array.from(
      new Set(
        submissionsWithGeo
          .map((s) => s.userAgent)
          .filter((ua) => typeof ua === 'string' && ua.trim().length > 0)
      )
    );

    const ipStatsMap = new Map();
    const uaStatsMap = new Map();
    const comboStatsMap = new Map();

    if (uniqueIps.length > 0) {
      const placeholders = uniqueIps.map((_, idx) => `$${idx + 1}`).join(',');
      const ipQuery = `
        SELECT 
          ip_address,
          COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '60 minutes') AS count_60,
          COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '10 minutes') AS count_10,
          MAX(created_at) AS last_seen
        FROM submissions
        WHERE ip_address IN (${placeholders})
        GROUP BY ip_address
      `;
      const ipStats = await db.execute(sql.raw(ipQuery, uniqueIps));
      for (const row of ipStats.rows || []) {
        ipStatsMap.set(row.ip_address, {
          recent60: Number(row.count_60 || 0),
          recent10: Number(row.count_10 || 0),
          lastSeenAt: row.last_seen,
        });
      }
    }

    if (uniqueUserAgents.length > 0) {
      const placeholders = uniqueUserAgents.map((_, idx) => `$${idx + 1}`).join(',');
      const uaQuery = `
        SELECT 
          user_agent,
          COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '60 minutes') AS count_60,
          ...
          MAX(created_at) AS last_seen
        FROM submissions
        WHERE user_agent IN (${placeholders})
        GROUP BY user_agent
      `;
      const uaStats = await db.execute(sql.raw(uaQuery, uniqueUserAgents));
      for (const row of uaStats.rows || []) {
        uaStatsMap.set(row.user_agent, {
          recent60: Number(row.count_60 || 0),
          recent10: Number(row.count_10 || 0),
          lastSeenAt: row.last_seen,
        });
    ...
    const uniqueCombos = Array.from(
      new Set(
        submissionsWithGeo
          .filter((s) => s.ipAddress && s.userAgent)
          .map((s) => JSON.stringify([s.ipAddress, s.userAgent]))
      )
    );
    ...
    if (uniqueCombos.length > 0) {
      const comboParams = [];
      let paramIndex = 1;
      const comboClauses = uniqueCombos.map((key) => {
        const [ip, ua] = JSON.parse(key);
        const clause = `(ip_address = $${paramIndex} AND user_agent = $${paramIndex + 1})`;
        comboParams.push(ip, ua);
        paramIndex += 2;
        return clause;
      });
      ...
      const comboQuery = `
        SELECT 
          ...
          MAX(created_at) AS last_seen
        FROM submissions
        WHERE ${comboClauses.join(' OR ')}
        GROUP BY ip_address, user_agent
      `;

      const comboStats = await db.execute(sql.raw(comboQuery, comboParams));
      for (const row of comboStats.rows || []) {
        const key = JSON.stringify([row.ip_address, row.user_agent]);
        comboStatsMap.set(key, {
          recent60: Number(row.count_60 || 0),
          recent10: Number(row.count_10 || 0),
          lastSeenAt: row.last_seen,
        });
    ...
    return res.json({
      submissions: submissionsWithSignals,
      total,
      pagination: {
        page,
        limit,
