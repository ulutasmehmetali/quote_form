...
export default function ServiceSelection({ onSubmit, initialData }: ServiceSelectionProps) {
  ...
  const fallbackCandidates: ServiceType[] = useMemo(() => {
    ...
    // hiçbir puanli eslesme yoksa en popüler iki servisi öner
  ...
  return (
    ...
    >
      ...
      {!searchQuery && (
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm font-semibold text-slate-800 shadow-sm hover:border-slate-300 transition w-fit self-start"
          >
            <span>Or browse all services</span>
            <svg
              className={`h-4 w-4 text-slate-500 transition-transform ${showBrowse ? 'rotate-180' : ''}`}
              viewBox="0 0 20 20"
              fill="none"
      ...
      <div className="pt-4 space-y-4">
        ...
        <div className="space-y-3">          <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
