...
export default function ServiceSelection({ onSubmit, initialData }: ServiceSelectionProps) {
  ...
  }, [searchQuery, fallbackSuggestions]);

  const displayedSuggestions = useMemo(() => {
    if (aiSuggestions.length > 0) {
      return aiSuggestions.slice(0, 2).map((s) => ({
        service: s.service,
        reason: s.reason || SERVICE_REASONS[s.service],
      }));
    }
    if (searchQuery && fallbackSuggestions.length > 0) {
      return fallbackSuggestions.slice(0, 2).map((s) => ({
        service: s,
        reason: SERVICE_REASONS[s],
      }));
    }
    return [];
  }, [aiSuggestions, searchQuery, fallbackSuggestions]);
